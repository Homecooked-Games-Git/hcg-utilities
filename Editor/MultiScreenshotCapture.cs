...
using UnityEngine;
	...
	public class MultiScreenshotCapture : EditorWindow
	{
		...
		private List<Vector2>
			resolutions = new List<Vector2>() { new Vector2(1024, 768) }; // Not readonly to support serialization
		...
		private void OnGUI()
		{
			...
			if (GUILayout.Button("Capture Screenshots"))
			{
				...
				{
					Camera camera = targetCamera == TargetCamera.GameView
						? Camera.main
						: SceneView.lastActiveSceneView.camera;
					CaptureScreenshot(new Vector2(camera.pixelWidth / camera.rect.width,
						camera.pixelHeight / camera.rect.height));
				}
		...
		private void CaptureQueuedScreenshots()
		{
			...
			else
			{
				...
				if (Vector2.Distance(new Vector2(resolution.width, resolution.height),
					    new Vector2(renderTex.width, renderTex.height)) > 15f)
					return;
